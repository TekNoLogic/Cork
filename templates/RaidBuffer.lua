...
if Cork.IHASCAT then
	function Cork:GenerateRaidBuffer(spellname, icon)
		...
		function dataobj:CorkIt(frame, playersonly)
			...
			for i=1,GetNumPartyMembers() do num = num + (dataobj["party"..i] and (IsSpellInRange(spellname, "party"..i) or IsSpellInRange(spellname, "party"..i)) and 1 or 0) end
			if num >= Cork.dbpc.multithreshold then return frame:SetManyAttributes("type1", "spell", "spell", spellname, "unit", "player") end
			...
			for i=1,GetNumRaidMembers() do
				...
				raidneeds[subgroup] = (raidneeds[subgroup] or 0) + (dataobj["raid"..i] and zone and online and not dead and (IsSpellInRange(spellname, "raid"..i) or IsSpellInRange(spellname, "raid"..i)) and 1 or 0)
				if raidneeds[subgroup] >= Cork.dbpc.multithreshold then return frame:SetManyAttributes("type1", "spell", "spell", spellname, "unit", "raid"..i) end
			end
